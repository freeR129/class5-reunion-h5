...
import { getMyVote, getVoteResults, submitVote, type VoteResults, type VoteStatus } from './lib/votes';
...
type VoteUiState = 'initializing' | 'idle' | 'submitting' | 'success' | 'error';
...
export default function Home() {
  ...
  useEffect(()=>{Promise.all([getVoteResults(),getMyVote()]).then(([realResults,myVote])=>{setResults(realResults);console.debug('results loaded',realResults);if(myVote.voted&&myVote.status){setSelectedVote(myVote.status);localStorage.setItem(confirmedVoteStorageKey,myVote.status);setVoteState('success');return}localStorage.removeItem(confirmedVoteStorageKey);setSelectedVote(null);setVoteState('idle')}).catch(error=>{console.error('[votes] Failed to initialize CloudBase vote state',error);setSelectedVote(null);setVoteState('error')})},[]);
  ...
  return <main className={`h5 page-${page}`} onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp} onWheel={onWheel}>
    ...
    <div className="pages" style={{transform:`translate3d(0,-${page*25}%,0)`}}>
      ...
      <section className="page vote-page" aria-label="聚会投票">{voteState==='initializing'?<div className="vote-panel"><p className="vote-loading" role="status" aria-live="polite">正在读取投票状态…</p></div>:voteState!=='success'?<div className="vote-panel"><h2>十年之后，<br/>你会来吗？</h2><p className="subtitle">2026，我们再聚一次。</p><div className="choices"><VoteButton code="A" title="算我一个，风雨无阻" selected={selectedVote==='attend'} disabled={voteState==='submitting'} onClick={()=>castVote('attend')}/><VoteButton code="B" title="虽然很想来，但实在排不开，心与你们同在" selected={selectedVote==='absent'} disabled={voteState==='submitting'} onClick={()=>castVote('absent')}/><VoteButton code="C" title="先观望~还不确定，晚点给准信" selected={selectedVote==='maybe'} disabled={voteState==='submitting'} onClick={()=>castVote('maybe')}/></div><p className="vote-loading" role="status" aria-live="polite">{voteState==='submitting'?'正在收好你的回答…':voteState==='error'?'提交失败，请检查网络后重试':'点击选项即完成投票 · 同一设备可修改'}</p></div>:<div className="success"><div className="success-message"><p className="success-index">2016 · 2026</p><h2>收到！</h2><div className="success-copy"><p>十年以前，<br/>我们在毕业照里站在一起。</p><p>十年以后，<br/>希望这一次，我们还能再见。</p></div><p className="success-ending">期待再次见面</p></div><div className="result-footer"><p className="result-kicker">已有</p><div className="result-grid"><div><strong>{results.absent}</strong><span>人<br/>遗憾缺席</span></div><i/><div><strong>{results.attend}</strong><span>人<br/>确认参加</span></div><i/><div><strong>{results.maybe}</strong><span>人<br/>还不确定</span></div></div><button onClick={modifyVote}>修改选择</button></div></div>}</section>
