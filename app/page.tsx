...
export default function Home() {
  ...
  return <main className={`h5 page-${page}`} onTouchStart={onStart} onTouchMove={onMove} onTouchEnd={onEnd} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp} onWheel={onWheel}>
    ...
    </div>{page===1&&<SwipeHint text="上滑回到当年" className="video-swipe-overlay"/>}<nav className="dots" aria-label="页面导航">{[0,1,2,3].map(i=><button key={i} className={page===i?'active':''} onClick={()=>goToPage(i)} aria-label={`前往第${i+1}页`}/>)}</nav><PhotoLightbox preview={photoPreview} onClose={closePhotoPreview}/>
  </main>;
}
