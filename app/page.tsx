...
export default function Home() {
  const [page,setPage]=useState(0), [opened,setOpened]=useState(false), [vote,setVote]=useState<Vote|null>(null), [videoError,setVideoError]=useState(false), [activeNode,setActiveNode]=useState(0);
  ...
  const selectNode=(index:number)=>{const track=getTrack(),slide=track?.querySelectorAll<HTMLElement>('.memory-slide')[index];if(track&&slide){track.scrollTo({left:slide.offsetLeft-(track.clientWidth-slide.clientWidth)/2,behavior:'smooth'});setActiveNode(index)}};
  const syncTimeline=()=>{const track=getTrack();if(!track)return;const center=track.scrollLeft+track.clientWidth/2;let closest=0,distance=Infinity;track.querySelectorAll<HTMLElement>('.memory-slide').forEach((slide,index)=>{const d=Math.abs(slide.offsetLeft+slide.clientWidth/2-center);if(d<distance){distance=d;closest=index}});setActiveNode(closest)};
  ...
  return <main className="h5" onTouchStart={onStart} onTouchEnd={onEnd} onMouseDown={onMouseDown} onMouseMove={onMouseMove} onMouseUp={onMouseUp} onMouseLeave={onMouseUp} onWheel={onWheel}>
    <div className="pages" style={{transform:`translate3d(0,-${page*100}dvh,0)`}}>
      ...
      <section className="page vote-page" aria-label="聚会投票"><div className="vote-orbit" aria-hidden="true"><span>2013</span><i>10</i><span>2026</span></div>{!vote?<div className="vote-panel"><p className="eyebrow">04 · THE PROMISE</p><h2>十年之后，<br/>你会来吗？</h2><p className="subtitle">2026，我们再聚一次。</p><div className="choices"><VoteButton code="A" title="我一定来" note="一定要认出彼此" onClick={()=>castVote('attend')}/><VoteButton code="B" title="尽量赶来" note="跨过山海来见你" onClick={()=>castVote('maybe')}/><VoteButton code="C" title="这次遗憾缺席" note="下一次，不缺席" onClick={()=>castVote('absent')}/></div><small>点击选项即完成投票 · 同一设备可修改</small></div>:<div className="success"><p className="result-kicker">已有</p><div className="result-grid"><div><strong>{results.attend}</strong><span>人<br/>确认参加</span></div><i/><div><strong>{results.maybe}</strong><span>人<br/>还在考虑</span></div></div><h2>期待再次见面</h2><span>你的选择已被珍藏</span><button onClick={()=>{localStorage.removeItem('reunion-vote');setVote(null)}}>修改选择</button></div>}<div className="down-hint">⌄<span>下滑回望</span></div></section>
