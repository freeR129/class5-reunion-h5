...
const timelineData = [
  ...
  {year:'2026',events:[{date:'2026.10.5',title:'再聚',description:'毕业十年，五班再聚',images:['/assets/memories/2026年10月5日-十周年聚会邀请函.webp']}]},
];
...
export default function Home() {
  ...
  const timelineWarmupImagesRef=useRef<HTMLImageElement[]>([]);
  ...
  const startVideoPreload=(source:'click'|'swipe'|'navigation'|'page-enter')=>{const video=videoRef.current;if(!video||videoPreloadStartedRef.current)return;videoPreloadStartedRef.current=true;video.muted=true;video.playsInline=true;video.setAttribute('playsinline','');video.setAttribute('webkit-playsinline','true');if(video.readyState<HTMLMediaElement.HAVE_CURRENT_DATA){console.debug(`[media] video preload start source=${source}`);video.load()}};
  const startTimelineWarmup=()=>{if(timelineWarmupStartedRef.current)return;timelineWarmupStartedRef.current=true;timelineWarmupTimerRef.current=window.setTimeout(()=>{console.debug('[media] timeline warmup start');timelineWarmupImagesRef.current=memoryNodes.slice(0,2).flatMap(node=>node.images).slice(0,6).map(src=>{const image=new Image();image.decoding='async';image.fetchPriority='low';image.src=src;return image})},120)};
  ...
  const enterVideoPage=(source:'click'|'swipe'|'navigation')=>{console.debug(`enter page2 source=${source}`);startVideoPreload(source);activePageRef.current=1;setPage(1);activateVideoPage(source);tryPlayAudio()};
  ...
  useEffect(()=>{activePageRef.current=page;const video=videoRef.current;if(!video)return;if(page===1){startVideoPreload('page-enter');activateVideoPage('page-enter')}else{video.pause();if(page===0)video.currentTime=0}if(page===2)console.debug('currentPage=2 timeline enabled=true')},[page]);
  useEffect(()=>()=>{window.clearTimeout(timelineWarmupTimerRef.current);timelineWarmupImagesRef.current=[]},[]);
